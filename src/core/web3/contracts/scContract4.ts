...
export class ContractSC4 {
  static async mintWithPrice(
    nft: Nft,
    ...
    const gasEstimated = await contract.estimateGas.mintWithPrice(
      nft.metaDataUrl
    );
    const gas = await getGasFees();
    console.log("gas", gas);
    const tx = await contract.mintWithPrice(nft.metaDataUrl, {
      from: account,
      value: nft.price,
  ...
  static async listNFT(
    ...
    try {
      ...
      let owner = await ContractSC4.getOwner(
        nft.tokenId,
        collection.contractAddress!,
        signer!
      );
      ...
      const isApproved = await ContractSC4.isApprovedForAll(
        collection,
        signer!
      );
      if (!isApproved) {
  ...
  static async setApprovedForAll(
    ...
    console.log("setApprovedForAll", collection, value);
    const contract = new ethers.Contract(
      collection.contractAddress!,
      SCPolygon3_ABI,
      signer
    ) as SimpleERC721;
    const gasEstimated = await contract.estimateGas.setApprovalForAll(
      collection.saleContract!,
      value
    );
